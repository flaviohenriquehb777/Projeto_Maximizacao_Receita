...
def testar_modelos_com_undersampling(df, target, test_size=0.2, random_state=42):
    ...
    for nome_alg, modelo in algoritmos.items():
        ...
        resultados.append({"modelo": nome_alg, "pr_auc": pr_auc})

    # Ordenando pelos três melhores modelos com base no PR AUC
    melhores_resultados = sorted(resultados, key=lambda x: x["pr_auc"], reverse=True)[
        :3
    ]

    return (
        f"1º Lugar: {melhores_resultados[0]['modelo']} - PR AUC: {melhores_resultados[0]['pr_auc']:.4f}\n"
